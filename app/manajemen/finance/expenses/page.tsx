              ...
              {allExpenses.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-10 text-center text-text-faint">
                    Belum ada data pengeluaran operasional pada periode {periodMonth}.
                  </td>
                </tr>
              ) : (
                allExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-surface-2/60">
                    <td className="px-3.5 py-2.5 font-mono font-bold text-gold-bright">{exp.expenseNumber}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{exp.date}</td>
                    <td className="px-3.5 py-2.5 font-semibold text-text">{exp.branchName}</td>
                    <td className="px-3.5 py-2.5">{getCategoryBadge(exp.category)}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{exp.recipientOrVendor || "-"}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{exp.paymentMethod}</td>
                    <td className="px-3.5 py-2.5 text-right font-mono font-bold text-danger">
                      {formatRupiah(exp.amount)}
                    </td>
                    <td className="max-w-xs px-3.5 py-2.5 truncate text-text-muted">{exp.notes}</td>
