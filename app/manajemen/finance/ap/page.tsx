              ...
              {allAP.length === 0 ? (
                <tr>
                  <td colSpan={10} className="py-10 text-center text-text-faint">
                    Belum ada data hutang dagang supplier.
                  </td>
                </tr>
              ) : (
                allAP.map((ap) => (
                  <tr key={ap.id} className="hover:bg-surface-2/60">
                    <td className="px-3.5 py-2.5 font-mono font-bold text-gold-bright">{ap.poNumber}</td>
                    <td className="px-3.5 py-2.5 font-semibold text-text">{ap.supplierName}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{ap.branchName}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{ap.receivedDate}</td>
                    <td className="px-3.5 py-2.5 font-mono font-semibold text-text-muted">{ap.dueDate}</td>
                    <td className="px-3.5 py-2.5 text-right font-mono text-text">{formatRupiah(ap.totalAmount)}</td>
                    <td className="px-3.5 py-2.5 text-right font-mono text-ok">{formatRupiah(ap.paidAmount)}</td>
                    <td className="px-3.5 py-2.5 text-right font-mono font-bold text-danger">
                      {formatRupiah(ap.remainingBalance)}
                    </td>
                    <td className="px-3.5 py-2.5">{getStatusBadge(ap.status)}</td>
