              ...
              {filteredCandidates.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-10 text-center text-text-faint">
                    Tidak ada kandidat customer reminder yang sesuai kriteria.
                  </td>
                </tr>
              ) : (
                filteredCandidates.map((c) => (
                  <tr key={c.customer.id} className="hover:bg-surface-2/60">
                    <td className="px-3.5 py-2.5">
                      <div className="font-semibold text-text">{c.customer.name}</div>
                      <div className="font-mono text-[11px] text-text-muted">{c.customer.phone}</div>
                    </td>
                    <td className="px-3.5 py-2.5">
                      {c.customer.tier ? (
                        <Badge tone="gold">{c.customer.tier}</Badge>
                      ) : (
                        <span className="text-text-faint">-</span>
                      )}
                    </td>
                    <td className="px-3.5 py-2.5">
                      <div className="font-semibold text-text">{c.lastVisitDate}</div>
                      <div className="text-[10px] text-text-faint">{c.daysSinceLastVisit} hari yang lalu</div>
                    </td>
                    <td className="px-3.5 py-2.5">
                      {c.type === "upcoming_appointment" ? (
                        <div>
                          <div className="font-semibold text-ok">
                            {c.upcomingAppointmentDate} · {c.upcomingAppointmentTime}
                          </div>
                          <div className="text-[10px] text-text-muted">{c.preferredBarberName ?? "Barber On-Duty"}</div>
                        </div>
                      ) : (
                        <div>
                          <div className="text-text">{c.preferredBarberName ?? "Belum ada favorit"}</div>
                          <div className="text-[10px] text-text-faint">{c.lastBranchName ?? "Semua Cabang"}</div>
                        </div>
                      )}
                    </td>
                    <td className="px-3.5 py-2.5">{getReminderTypeBadge(c.type)}</td>
                    <td className="px-3.5 py-2.5">
                      {c.isEligible ? (
                        <Badge tone="ok">Siap Kirim</Badge>
                      ) : (
                        <Badge tone="neutral">{c.ineligibilityReason ?? "Terkirim Baru-baru Ini"}</Badge>
                      )}
                    </td>
                    <td className="max-w-xs px-3.5 py-2.5">
                      <div className="truncate italic text-text-muted" title={c.suggestedMessage}>
                        &quot;{c.suggestedMessage}&quot;
                      </div>
                    </td>
                    <td className="px-3.5 py-2.5 text-center">
                      <Button
                        variant={c.isEligible ? "primary" : "ghost"}
