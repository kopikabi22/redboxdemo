              ...
              {filteredLogs.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-text-faint">
                    Belum ada rekaman log audit yang sesuai dengan filter.
                  </td>
                </tr>
              ) : (
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-surface-2/60">
                    <td className="whitespace-nowrap px-3.5 py-2.5 font-mono text-text-muted">
                      {formatTimestamp(log.timestamp)}
                    </td>
                    <td className="px-3.5 py-2.5">
                      <div className="font-bold text-text">{log.actorName}</div>
                      <div className="text-[10px] text-text-faint">{log.actorRole}</div>
                    </td>
                    <td className="px-3.5 py-2.5 text-text-muted">{log.branchName}</td>
                    <td className="px-3.5 py-2.5">{getActionBadge(log.action)}</td>
                    <td className="px-3.5 py-2.5 font-mono">
                      <span className="text-text-muted">{log.entityType}:</span>{" "}
                      <span className="font-bold text-gold-bright">{log.entityId}</span>
                    </td>
                    <td className="max-w-md px-3.5 py-2.5 text-text">{log.details}</td>
