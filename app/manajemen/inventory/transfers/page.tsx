              ...
              {filteredTransfers.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-10 text-center text-text-faint">
                    Belum ada riwayat Stock Transfer antar-cabang.
                  </td>
                </tr>
              ) : (
                filteredTransfers.map((tr) => (
                  <tr key={tr.id} className="hover:bg-surface-2/60">
                    <td className="px-3.5 py-2.5 font-mono font-bold text-gold-bright">{tr.transferNumber}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{tr.createdAt.slice(0, 10)}</td>
                    <td className="px-3.5 py-2.5 font-medium text-text">{tr.sourceBranchName}</td>
                    <td className="px-3.5 py-2.5 font-medium text-text">{tr.targetBranchName}</td>
                    <td className="px-3.5 py-2.5 text-center font-mono font-bold">{tr.totalQty} Unit</td>
                    <td className="px-3.5 py-2.5 text-right font-mono font-bold text-text">
                      {formatRupiah(tr.totalValue)}
                    </td>
                    <td className="px-3.5 py-2.5">{getStatusBadge(tr.status)}</td>
                    <td className="px-3.5 py-2.5 text-center">
                      <Button
                        variant="ghost"
                        className="px-2.5 py-1 text-[11px]"
                        onClick={() => setDetailTransfer(tr)}
                      >
                        Detail &amp; Aksi
                      </Button>
                    </td>
