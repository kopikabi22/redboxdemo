              ...
              {filteredOpnames.length === 0 ? (
                ...
                filteredOpnames.map((op) => (
                  <tr key={op.id} className="hover:bg-surface-2/60">
                    <td className="px-3.5 py-2.5 font-mono font-bold text-gold-bright">{op.opnameNumber}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{op.opnameDate}</td>
                    <td className="px-3.5 py-2.5 font-medium text-text">{op.branchName}</td>
                    <td className="px-3.5 py-2.5 text-center font-mono">{op.items.length}</td>
                    <td className="px-3.5 py-2.5 text-center font-mono">
                      {op.totalVarianceItems > 0 ? (
                        <span className="font-bold text-warn">{op.totalVarianceItems} Item</span>
                      ) : (
                        <span className="text-ok">0 (Sesuai)</span>
                      )}
                    </td>
                    <td className="px-3.5 py-2.5 text-center font-mono font-bold">
                      {op.totalVarianceQty > 0 ? (
                        <span className="text-ok">+{op.totalVarianceQty}</span>
                      ) : op.totalVarianceQty < 0 ? (
                        <span className="text-danger">{op.totalVarianceQty}</span>
                      ) : (
                        <span className="text-text-muted">0</span>
                      )}
                    </td>
                    <td className="px-3.5 py-2.5">{getStatusBadge(op.status)}</td>
                    <td className="px-3.5 py-2.5 text-text-muted">{op.conductedByName}</td>
